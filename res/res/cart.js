var cart = {
  ...
  init : function () {
    ...
    for (let id in res) {
      ...
      item.appendChild(part);
	        ...
	        part = document.createElement("div");
      part.innerHTML = "Table is :"+p.cind;
      part.className = "p-pric";
      item.appendChild(part);
  ...
  // (D) LIST CURRENT CART ITEMS (IN HTML)
   ...
   part = document.createElement("div");
        part.innerHTML =" Table's number is  : "+p.number;
        part.className = "c-name";
        item.appendChild(part);

        // NAME
        part = document.createElement("div");
        part.innerHTML = "resturent's name : "+p.name;
        part.className = "c-name";
        item.appendChild(part);
  ...
  change : function () {
    ...
    // (F2) UPDATE TOTAL ONLY
      ...
      var total = 0;
      for (let id in cart.items) {
        total += cart.items[id] * res[id].price;
